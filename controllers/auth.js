...
const login = async ( req, res) => {
    try {
        ...
        if(user == null) return res.send({msg:'Please visit the cee department to get a free account'})
        if(user.isBlocked) return res.send({ msg:'Please visit the school admin '})
...
const getUser = async ( req, res) => {
   try {
     ...
     if (userId == null) return res.status(500).send({ msg: "You are not logged in !!" })
